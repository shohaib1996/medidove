...
export const replyToLead = async (formData: FormData) => {
  ...
  await admin.from("consent_logs").insert({
    ...
    channel: "email",
    consented: true,
    reason: "Implied consent: reply to an inbound contact form inquiry.",
  });
  ...
  await writeAuditLog(admin, {
    ...
    entityType: "contact_leads",
    entityId: leadId,
    summary: `Sent AI-assisted reply to ${lead.name} (${lead.email}).`,
    metadata: { outcome: dispatch.outcome, provider: dispatch.result.provider },
  });

  if (dispatch.outcome === "sent" && lead.status === "new") {
    await admin.from("contact_leads").update({ status: "contacted" }).eq("id", leadId);
  }
